...
//	[self performSelector:@selector(hideDelayed:) withObject:[NSNumber numberWithBool:delay] afterDelay:delay];
    self.hideTimer = [NSTimer scheduledTimerWithTimeInterval:delay target:self  selector:@selector(hideThisView)                                            userInfo:nil repeats:NO];;
}
...
- (void)setTransformForCurrentOrientation:(BOOL)animated {
	UIDeviceOrientation orientation = 0;
	NSInteger degrees = 0;
