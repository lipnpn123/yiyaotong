                                                      ...
                                                      forString:(NSString*)aString
{
    ...
    [attrString addAttribute:(NSString *)kCTFontAttributeName
                       value:(id)CFBridgingRelease(CTFontCreateWithName((CFStringRef)CFBridgingRetain([UIFont systemFontOfSize:14].fontName),
                                                                        14,
                                                                        NULL))
                       range:NSMakeRange(0, [aString length])];
    if (self.colorLength > [aString length])
    {
        self.colorLength = [aString length];
    }
    [attrString addAttribute:(NSString *)kCTForegroundColorAttributeName
                       value:(id)[UIColor greenColor].CGColor
                       range:NSMakeRange(0, self.colorLength)];
...
-(float)drawThisView
{
    ...
    NSLog(@"CGContextRestoreGState  %f",y);
    // 恢复 context 信息
    totoalHeigth = -y;
