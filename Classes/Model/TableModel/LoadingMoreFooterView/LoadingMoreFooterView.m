...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.textLabel.text = NSLocalizedString(@"上拉获取更多", @"Legen Sie mehr");
...
- (void) setShowActivityIndicator:(BOOL)showActivityIndicator {
    ...
    if (showActivityIndicator && !self.activityView) {
        ...
        self.activityView.center = CGPointMake(self.frame.size.width-70, self.frame.size.height / 2);
        [self addSubview:self.activityView];
        [self.activityView startAnimating];
        self.textLabel.text = NSLocalizedString(@"正在请求...", @"Laden");
    }
...
{
	if (delegate !=nil && [delegate respondsToSelector:@selector(footViewTouch:)]) 
	{
		[delegate performSelector:@selector(footViewTouch:) withObject:self ];
	}
}
